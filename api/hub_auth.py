"""
Token management for Amplex.
Signs HS256 JWTs locally using SECRET_KEY — no external SSO dependency.
"""
...
# ── Access token (HS256, platform-local) ─────────────────
...
def create_access_token(user_id: str, extra_claims: dict | None = None) -> str:
    """Create an HS256 access token for the given user."""
    now = int(time.time())
    payload = {
        "sub": str(user_id),
        "type": "access",
...
# ── Refresh token (HS256, platform-internal) ─────────────
