...
# ── S2S API Key Endpoints ────────────────────────────────


class TestS2SEndpoints:
    """Service-to-service API key protection."""

    def test_metrics_without_api_key_rejected(self, client):
        resp = client.get("/amplex/api/metrics")
        assert resp.status_code in (401, 403)

    def test_metrics_wrong_api_key(self, client):
        resp = client.get("/amplex/api/metrics", HTTP_X_API_KEY="wrong-key")
        assert resp.status_code in (401, 403)

    def test_create_opportunity_without_key(self, client):
        resp = client.post(
            "/amplex/api/opportunities",
            data=json.dumps({"title": "Test Opp", "client_slug": "fake"}),
            content_type="application/json",
        )
        assert resp.status_code in (401, 403)

    def test_import_contacts_without_key(self, client):
        resp = client.post(
            "/amplex/api/contacts/import",
            data=json.dumps({"client_slug": "fake", "contacts": []}),
            content_type="application/json",
        )
        assert resp.status_code in (401, 403)

    def test_opportunity_stage_update_without_key(self, client):
        resp = client.put(
            "/amplex/api/opportunities/fake-id/stage",
            data=json.dumps({"stage": "won"}),
            content_type="application/json",
        )
        assert resp.status_code in (401, 403, 404, 405)

    def test_contacts_search_without_key(self, client):
        resp = client.get("/amplex/api/contacts/search?q=test")
