...
function priorityStars(p: string) {
  ...
  if (n <= 0) return null;
  return <>{Array.from({ length: n }, (_, i) => <Star key={i} size={11} fill="currentColor" />)}</>;
}
...
export default function Pipeline() {
  ...
  const [appliedFilters, setAppliedFilters] = useState({ search: '', user_id: '', min_value: '', max_value: '' });
  ...
  return (
    <div className="page">
      ...
      <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1rem", flexWrap: "wrap", alignItems: "flex-end" }}>
        <div>
          <input
            ...
            onKeyDown={(e) => { if (e.key === 'Enter') setAppliedFilters({ ...appliedFilters, search: searchText }); }}
        ...
        {isManager && (
          <div>
            <select
              className="select"
              value={filterUser}
              onChange={(e) => { setFilterUser(e.target.value); setAppliedFilters({ ...appliedFilters, user_id: e.target.value }); }}
        ...
        <button
          ...
          onClick={() => setAppliedFilters({ search: searchText, user_id: filterUser, min_value: minValue, max_value: maxValue })}
        >
          Filtrar
        </button>
        {(appliedFilters.search || appliedFilters.user_id || appliedFilters.min_value || appliedFilters.max_value) && (
          <button
            className="btn btn-ghost btn-sm"
            style={{ fontSize: "0.8rem", color: "var(--danger)" }}
            onClick={() => { setSearchText(''); setFilterUser(''); setMinValue(''); setMaxValue(''); setAppliedFilters({ search: '', user_id: '', min_value: '', max_value: '' }); }}
          >
            Limpar
          </button>
        )}
          ...
          >
            <div className="kanban-column-header">
              <span className="kanban-column-title">
                {col.name}
                {col.is_won && <span style={{ color: "var(--success)", marginLeft: "0.4rem", display: "inline-flex" }}><Check size={14} /></span>}
              </span>
                ...
                >
                  <div className="kanban-card-title">
                    {priorityStars(card.priority) !== null && (
                      <span style={{ color: "var(--warning)", marginRight: "0.35rem", display: "inline-flex", gap: "1px" }}>
                  ...
                  {card.tag_ids.length > 0 && (
                    <div style={{ marginTop: "0.4rem", display: "flex", gap: "0.3rem", flexWrap: "wrap" }}>
                      {card.tag_ids.map((tag) => (
                        <span key={tag.id} className="badge badge-info" style={{ fontSize: "0.65rem" }}>
                          {tag.name}
                        </span>
                      ))}
                    </div>
                  ...
                  {card.user_name && (
                    <div style={{ marginTop: "0.35rem", fontSize: "0.7rem", color: "var(--text-light)" }}>
                      {card.user_name}
                    </div>
                  )}
                </div>
      ...
      {/* Lost Reason Dialog */}
      {lostDialog && (
        <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.6)" }} onClick={() => setLostDialog(null)}>
          <div className="glass" style={{ width: "100%", maxWidth: 400, padding: "2rem" }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
              <h2 style={{ fontSize: "1.1rem", fontWeight: 700, color: "#fff" }}>Motivo da Perda</h2>
              <button className="btn btn-ghost" onClick={() => setLostDialog(null)} style={{ padding: "0.25rem 0.5rem" }}><X size={14} /></button>
            </div>
