...
const PRODUCTS: Product[] = [
  { key: "hub", name: "Hub", description: "Painel Central", url: "", icon: <Home size={16} />, color: "#0070ff" },
  { key: "amplex", name: "Amplex", description: "CRM", url: "", icon: <BarChart3 size={16} />, color: "#10b981" },
  { key: "nexus", name: "Nexus", description: "Atendimento", url: "", icon: <MessageSquare size={16} />, color: "#8b5cf6" },
  { key: "entity", name: "Entity", description: "Dados CNPJ", url: "", icon: <Search size={16} />, color: "#f59e0b" },
];
...
export default function ProductSwitcher() {
  ...
  useEffect(() => {
    // Fetch product URLs from Amplex config
    const token = localStorage.getItem("hub_token");
    if (!token) return;
    fetch("/amplex/api/crm/config", {
          ...
          {visible.map(p => {
            ...
            return (
              <a
                ...
                onMouseEnter={e => { if (!isCurrent && p.url) e.currentTarget.style.background = "rgba(255,255,255,0.06)"; }}
