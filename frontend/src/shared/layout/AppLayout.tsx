...
function NavIcon({ name }: { name: string }) {
  ...
  switch (name) {
    ...
    case "pipeline":
      return <svg {...props} viewBox="0 0 24 24"><rect x="2" y="3" width="5" height="18" rx="1" /><rect x="9.5" y="6" width="5" height="15" rx="1" /><rect x="17" y="9" width="5" height="12" rx="1" /></svg>;
    case "leads":
      return <svg {...props} viewBox="0 0 24 24"><path d="M22 12h-4l-3 9L9 3l-3 9H2" /></svg>;
    case "contacts":
      return <svg {...props} viewBox="0 0 24 24"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M22 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>;
    case "settings":
...
export default function AppLayout() {
  ...
  useEffect(() => {
    ...
    if (!token) return;
    fetch("/amplex/api/crm/config", { headers: { Authorization: `Bearer ${token}` } })
  ...
  function handleGlobalSearch(e: React.FormEvent) {
    ...
    if (globalSearch.trim()) {
      navigate(`/leads?search=${encodeURIComponent(globalSearch.trim())}&page=1`);
      setGlobalSearch('');
    }
  }
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      ...
      <div className="top-bar" style={{
        ...
        {platformItems.map(p => {
          ...
          return (
            <a
              ...
              onClick={e => { if (isCurrent || !p.url) e.preventDefault(); }}
              style={{
                ...
                transition: "all 0.2s", cursor: isCurrent ? "default" : (p.url ? "pointer" : "default"),
                opacity: p.url || isCurrent ? 1 : 0.4,
              }}
              onMouseEnter={e => { if (!isCurrent && p.url) { e.currentTarget.style.background = "rgba(255,255,255,0.06)"; e.currentTarget.style.color = "#fff"; } }}
              onMouseLeave={e => { if (!isCurrent) { e.currentTarget.style.background = isCurrent ? "rgba(0,112,255,0.12)" : "transparent"; e.currentTarget.style.color = isCurrent ? "#fff" : "rgba(255,255,255,0.55)"; } }}
            >
              <span style={{ fontSize: "0.85rem", display: "flex", alignItems: "center" }}>{p.icon}</span>
              <span>{p.name}</span>
            </a>
          );
        })}
        ...
        <form onSubmit={handleGlobalSearch} style={{ maxWidth: 400, display: "flex", gap: "0.5rem" }}>
          <div style={{ position: "relative" }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ position: "absolute", left: "0.75rem", top: "50%", transform: "translateY(-50%)" }}>
            ...
            <input
              ...
              style={{ paddingLeft: "2.25rem", height: "30px", fontSize: "0.8rem", width: "240px", minWidth: "120px", flex: "1" }}
            />
          </div>
        </form>

        <span className="badge badge-info" style={{ fontSize: "0.65rem" }}>{user.role === 'admin' ? 'Gestor' : 'Vendedor'}</span>
        ...
        <div ref={notifRef} style={{ position: "relative" }}>
          <button
            ...
            style={{
              ...
              color: badgeCount > 0 ? "#fff" : "var(--text-muted)", transition: "all 0.2s",
            }}
            title="Notificações"
          >
          ...
          {notificationsOpen && (
            <div style={{
              ...
              width: 380, maxWidth: "calc(100vw - 2rem)", maxHeight: 480, overflowY: "auto",
              background: "rgba(14, 17, 28, 0.97)",
              backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)",
              border: "1px solid rgba(45,56,71,0.5)", borderRadius: "12px",
              boxShadow: "0 8px 32px rgba(0,0,0,0.5)", zIndex: 100,
            }}>
              <div style={{
                padding: "0.85rem 1rem", borderBottom: "1px solid rgba(45,56,71,0.4)",
                display: "flex", justifyContent: "space-between", alignItems: "center",
              }}>
                <span style={{ fontSize: "0.9rem", fontWeight: 600, color: "#fff" }}>Notificações</span>
              ...
              {(!notifData || notifData.items.length === 0) ? (
                ...
                <div style={{ padding: "0.5rem" }}>
                  {notifData.items.map((notif) => {
                    const stateColor = notif.state === 'overdue' ? 'var(--danger)' : notif.state === 'today' ? 'var(--warning)' : 'var(--info)';
                    const stateLabel = notif.state === 'overdue' ? 'Atrasada' : notif.state === 'today' ? 'Hoje' : notif.date_deadline ? new Date(notif.date_deadline + 'T12:00:00').toLocaleDateString('pt-BR') : '';
                    return (
                      <div key={notif.id} style={{
                        padding: "0.65rem 0.75rem", borderRadius: "8px", marginBottom: "0.35rem",
                        background: notif.state === 'overdue' ? "rgba(255,59,48,0.08)" : "rgba(255,255,255,0.02)",
                        border: `1px solid ${notif.state === 'overdue' ? 'rgba(255,59,48,0.2)' : 'rgba(45,56,71,0.3)'}`,
                      }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "0.5rem" }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: "0.8rem", fontWeight: 600, color: "#fff", marginBottom: "0.15rem" }}>
                              {notif.summary || 'Retorno agendado'}
                            </div>
                            <div
                              style={{ fontSize: "0.75rem", color: "var(--text-muted)", cursor: "pointer", textDecoration: "underline", textDecorationColor: "rgba(255,255,255,0.2)" }}
                              onClick={() => { navigate(`/leads/${notif.lead_id}`); setNotificationsOpen(false); }}
                            >
                              {notif.lead_name}
                            </div>
                            <div style={{ display: "flex", gap: "0.5rem", alignItems: "center", marginTop: "0.3rem" }}>
                              <span style={{ fontSize: "0.65rem", padding: "0.1rem 0.35rem", borderRadius: "4px", background: `${stateColor}20`, color: stateColor, fontWeight: 600 }}>
                                {stateLabel}
                              </span>
                              {notif.activity_type && (
                                <span style={{ fontSize: "0.65rem", color: "var(--text-muted)" }}>{notif.activity_type}</span>
                              )}
                            </div>
                          </div>
                          <div style={{ display: "flex", gap: "0.25rem", flexShrink: 0 }}>
                            <button
                              ...
                              style={{
                                ...
                                borderRadius: "6px", padding: "0.25rem 0.4rem", cursor: "pointer",
                            ...
                            <button
                              ...
                              style={{
                                background: "rgba(255,255,255,0.03)", border: "1px solid rgba(45,56,71,0.4)",
                                borderRadius: "6px", padding: "0.25rem 0.4rem", cursor: "pointer",
            ...
            {!collapsed ? (
              <>
                <div style={{ padding: "0.5rem 0.75rem", marginBottom: "0.5rem" }}>
                  <p style={{ color: "#fff", fontSize: "0.875rem", fontWeight: 500 }}>{user.name}</p>
      ...
      <main className="main-content" style={{ flex: 1, marginLeft: SIDEBAR_COLLAPSED, marginTop: TOP_BAR_HEIGHT, minHeight: `calc(100vh - ${TOP_BAR_HEIGHT}px)`, overflowY: "auto", display: "flex", flexDirection: "column" }}>
        <div style={{ flex: 1 }}>
          <Outlet />
        </div>
        <footer style={{
