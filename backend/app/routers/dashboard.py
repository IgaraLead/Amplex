...
from app.models import Contact, Interaction, Lead, OrgMember, Source, Stage, User

router = APIRouter(prefix="/amplex/api/o/{org_id}/crm", tags=["dashboard"])
...
def dashboard(
    ...
    def base_filter():
        q = [Lead.active.is_(True), Lead.org_id == current_user.org_id]
        if not is_manager:
    ...
    total_leads = (
        ...
        .filter(*base_filter(), Lead.type == "lead")
        .scalar()
        or 0
    )
    total_opps = (
        db.query(func.count(Lead.id))
        .filter(*base_filter(), Lead.type == "opportunity")
        .scalar()
        or 0
    )
    ...
    won_stages = (
        db.query(Stage.id)
        .filter(Stage.is_won.is_(True), Stage.org_id == current_user.org_id)
        .subquery()
    )
    won = (
        db.query(func.count(Lead.id))
    ...
    new_this_month = (
        db.query(func.count(Lead.id))
        .filter(*base_filter(), Lead.created_at >= month_start.isoformat())
        .scalar()
        or 0
    )
    new_last_month = (
    ...
    total_contacts = (
        ...
        .filter(Contact.active.is_(True), Contact.org_id == current_user.org_id)
        .scalar()
        or 0
    )
    ...
    for i in range(6):
        ...
        result = (
            ...
            .filter(
                ...
                Lead.stage_id.notin_(
                    [
                        s.id
                        for s in db.query(Stage)
                        .filter(
                            Stage.is_won.is_(True), Stage.org_id == current_user.org_id
                        )
                        .all()
                    ]
                )
