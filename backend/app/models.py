...
class Lead(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    type: Mapped[str] = mapped_column(String(16), default="opportunity")  # 'lead' | 'opportunity'
    ...
    stage: Mapped[Optional["Stage"]] = relationship(back_populates="leads", lazy="joined")
    ...
    custom_field_values: Mapped[list["CustomFieldValue"]] = relationship(back_populates="lead", cascade="all, delete-orphan")
...
class Interaction(Base):
    ...
    files: Mapped[list["InteractionFile"]] = relationship(back_populates="interaction", cascade="all, delete-orphan")
...
class Activity(Base):
    ...
    user: Mapped[Optional["User"]] = relationship(back_populates="activities", lazy="joined")
...
class CustomField(Base):
    ...
    __table_args__ = (UniqueConstraint("name", name="uq_custom_field_name"),)
...
class CustomFieldValue(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    lead_id: Mapped[int] = mapped_column(ForeignKey("leads.id", ondelete="CASCADE"), index=True)
    field_id: Mapped[Optional[int]] = mapped_column(ForeignKey("custom_fields.id", ondelete="CASCADE"))
